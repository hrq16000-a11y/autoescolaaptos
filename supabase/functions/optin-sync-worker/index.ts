...
//
// Modos:
//   GET /?test=1  -> healthcheck da conexão com Google Sheets. Registra o resultado
//                    em marketing_optin_sync_attempts com source='cron_test' (optin_id NULL).
//   GET /         -> processa fila de opt-ins não sincronizados.
...
async function loadAlertConfig() {
  const { data } = await admin
    ...
    .maybeSingle();
  return {
    threshold: data?.alert_queue_threshold ?? Number(Deno.env.get("ALERT_QUEUE_THRESHOLD") ?? 25),
...
async function runHealthcheck() {
  if (!SHEET_ID || !LOVABLE_API_KEY || !GOOGLE_SHEETS_API_KEY) {
    return { ok: false, status: 500, error: "missing_sheet_config" };
  }
  ...
  const ok = r.ok;
  const errorMsg = ok ? null : `[${r.status}] ${bodyText.slice(0, 300)}`;

  // Registra o healthcheck no histórico (optin_id NULL, source='cron_test').
  await admin.from("marketing_optin_sync_attempts").insert({
    optin_id: null,
    telefone: "__healthcheck__",
    ok,
  ...
  // Alerta quando o healthcheck falhar.
  if (!ok) {
    try {
      const cfg = await loadAlertConfig();
      if ((cfg.emailEnabled || cfg.slackEnabled) && shouldFire("healthcheck_failed", 60 * 60 * 1000)) {
        await sendAlert({
          title: "Healthcheck do Google Sheets falhou",
          message: `Status ${r.status}. Verifique credenciais/planilha.`,
          severity: "critical",
          meta: { status: r.status, error: errorMsg },
        });
      }
    } catch (e) { console.warn("healthcheck_alert_failed", e); }
  }

  return { ok, status: r.status, body: parsed ?? bodyText.slice(0, 500) };
}
...
Deno.serve(async (req) => {
  ...
  if (testOnly) {
    ...
    return new Response(JSON.stringify({
      ok: r.ok, status: r.status, sheet_id: SHEET_ID, tab: SHEET_TAB, body: r.body,
    }), {
      status: r.ok ? 200 : 502,
  ...
  try {
    const cfg = await loadAlertConfig();
    const alertsOn = cfg.emailEnabled || cfg.slackEnabled;
    const errorRate = rows.length ? failed / rows.length : 0;
    if (alertsOn && failed >= 3 && errorRate >= 0.5 && shouldFire("worker_error_rate")) {
      await sendAlert({
    ...
    const { count: pendingCount } = await admin
      ...
      .neq("sheet_sync_status", "ok");
    if (alertsOn && (pendingCount ?? 0) >= cfg.threshold && shouldFire("worker_queue_high")) {
      await sendAlert({
        title: "Fila de sincronização acima do limite",
        message: `Há ${pendingCount} opt-ins pendentes/erro (limite: ${cfg.threshold}).`,
        severity: "warning",
        meta: { pendingCount, threshold: cfg.threshold },
      });
    }
  } catch (e) {
